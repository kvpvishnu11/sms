...
/**
 * This controller handles the requests to sign up a new user, check if a user
 * exists, display profile using user name
 **/
...
public class UserLoginController {
	...
	public ResponseEntity<?> saveNewUserSignup(@RequestBody UserLogin u) {
		...
		if (u.getUsername() == null || u.getUsername().isEmpty() || u.getPassword() == null || u.getPassword().isEmpty()
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body("Please check the request JSON. Username/Password/Role is missing.");
		}
	...
	public ResponseEntity<?> checkIfUserIsPresent(@RequestBody UserDTO u) {
		...
		if (u.getUsername() == null || u.getUsername().isEmpty() || u.getPassword() == null
			...
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body("Please check the request JSON. Username/Password is missing.");
		}
	...
	public ResponseEntity<?> getUserDetails(@PathVariable("uname") String username) {
		...
		if (username == null || username.isEmpty()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
					.body("Please check the JSON request. Username is empty.");
		}
